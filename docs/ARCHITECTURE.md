...
| `ad-core` | lib | Game engine (the rules). Pure logic, no IO. Owns `GameState` (the shared mutation interface is its `can_x()`/`buy_x()` method surface), the `Action` IR + `apply_action` adapter for action-as-data consumers (`ad-sim`, Python), and the `data` module for static config. Never depends on `ad-sim`. File map: [`crates/ad-core/ARCHITECTURE.md`](../crates/ad-core/ARCHITECTURE.md). |
| `ad-sim` | lib | Simulation driver (decides *what a player does*). **Planned future feature, not under active development** — frozen at the pre-Infinity frontier (its `Action` vocabulary and `ObservedState` cover early-game strategy runs; integration tests disabled via `autotests = false`). Kept compiling; revive by extending `ObservedState`/`Action` one prestige layer at a time. One-way dependency on `ad-core`; mutates the game only by emitting `Action`s. Hosts the `Controller` trait, `StrategyController`, and `run_simulation`/`simulate`. See [`design/2026-06-27-simulation-architecture.md`](design/2026-06-27-simulation-architecture.md) and the file map in [`crates/ad-sim/ARCHITECTURE.md`](../crates/ad-sim/ARCHITECTURE.md). |
