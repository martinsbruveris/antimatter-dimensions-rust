//! The `GameView` snapshot: the serializable per-frame view of `GameState`
//! sent to the webview, its ~90 sub-view structs, and the `build_*_view`
//! builders. Read-only over the engine — commands live in `commands.rs`.
...
use ad_core::{
    AutobuyerMode, AutobuyerTarget, Decimal, GameState, ALL_BREAK_INFINITY_REBUYABLES,
    ALL_BREAK_INFINITY_UPGRADES, ALL_INFINITY_UPGRADES, INFINITY_CHALLENGE_COUNT,
    INFINITY_DIMENSION_COUNT, NORMAL_CHALLENGE_COUNT, REPLICANTI_UNLOCK_COST,
};
use serde::Serialize;
...
pub(crate) struct GameView {
    ...
    /// Automator tab state (Feature 6.6 Stage D). `pub(crate)`: the
    /// tick command drains the queued notify toasts into it per frame.
    pub(crate) automator: AutomatorTabView,
...
#[derive(Serialize)]
pub(crate) struct AutomatorTabView {
...
pub(crate) fn build_game_view(game: &GameState) -> GameView {
...
pub(crate) fn me_to_num(me: Option<(f64, f64)>) -> Num {
    match me {
        Some((m, e)) => Num { m, e },
