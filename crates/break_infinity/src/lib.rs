...
pub fn afford_geometric_series(
    ...
    Decimal::from_float(
        (resources_available / actual_start * (price_ratio - Decimal::from_float(1.0))
...
pub fn sum_geometric_series(
    ...
    price_start
        * price_ratio.pow(current_owned)
        * (Decimal::from_float(1.0) - price_ratio.pow(num_items))
        / (Decimal::from_float(1.0) - price_ratio)
}
...
pub fn afford_arithmetic_series(
    ...
    let b = actual_start - (price_add / Decimal::from_float(2.0));
    let b2 = b.pow(&Decimal::from_float(2.0));
    (b.neg()
        + ((b2 + ((price_add * resources_available) * Decimal::from_float(2.0))).sqrt()
            / price_add))
        .floor()
}
...
pub fn sum_arithmetic_series(
    ...
    num_items / Decimal::from_float(2.0)
        * (actual_start * Decimal::from_float(2.0)
            + (num_items - Decimal::from_float(1.0))
            + num_items
            - Decimal::from_float(1.0))
        * price_add
}
